...
function model = rumen()
    % Mathematical model of rumen fermentation
    ...
    % Parameters
     ...
     %syms  khyd_nsc khyd_pro km_su Ysu km_aa Yaa km_h2 Yh2  lambda_1 lambda_2 
     %khyd_ndf = 0.05; 
     %Ks_su = 9e-3; 
	 %Ks_aa = 6.4e-3; 
	 %Ks_h2 = 5.84e-6;  
...
% Microbial kinetic rates
     ...
     %model.sym.p = [khyd_ndf; khyd_nsc; khyd_pro; km_su; Ks_su; Ysu; km_aa; Ks_aa; Yaa; km_h2; Ks_h2; Yh2;  lambda_1; lambda_2];
     model.sym.p = [khyd_nsc; khyd_pro; km_su; Ysu; km_aa; Yaa; km_h2; Yh2;  lambda_1; lambda_2];
 
     
    
     % State variables
     model.sym.x = [z_ndf; z_nsc; z_pro; s_su; s_aa; s_ac; s_bu; s_pr; s_IN; s_IC; s_h2; s_ch4; x_su; x_aa; x_h2; ng_co2; ng_h2; ng_ch4];
  
    
    % Functions 
     roh   = km_h2*exp(-Ks_h2*V_g/(ng_h2+eps))*x_h2*V_l; %Thermodynamic kinetics wrt h2 concentration in the gas phase, molH2/h
     rho_xh2 = k_d*x_h2; 
     rhoT_co2 = kLa*(s_co2 - KH_co2*R*Trumen*ng_co2/V_g); 
    ...
    % Initial conditions
    Cinit = [    2.4920    3.4939    1.2933    0.0007         0    0.0263    0.0051    0.0074    0.0075    0.1404    0.0000    0.0007    0.0910    0.0048    0.0010 0 0 0]; 
    model.sym.x0  = Cinit'; 
    
    % Observables
    model.sym.y = [z_ndf; z_nsc; z_pro; s_su; s_aa; s_ac; s_bu; s_pr;s_IN; ng_co2; ng_h2; ng_ch4 ]; 
    %model.sym.y = [s_ac; s_bu; s_pr;s_IN; ng_co2; ng_h2; ng_ch4 ]; 
